...
$sql="select * from (";
$sql.="Select PK_DealerID as dealerid,DealerName as dealername,cwProductName as brandname,FK_LocationID as LocationID,FK_CompanyID as CompanyID from tbl_master_dealerinformation Where cwExecutiveName in(".$sql_salesexecutivelist.") and FK_LocationID=".$locationid." and FK_CompanyID=".$companyid ."  and IsActive=1 ";
$sql.=" union all  Select PK_DealerID as dealerid,DealerName as dealername,cwProductName as brandname,FK_LocationID as LocationID,FK_CompanyID as CompanyID from tbl_master_dealerinformation Where cwExecutiveName like '".$username1."' and FK_LocationID=".$locationid." and FK_CompanyID=".$companyid ."  and IsActive=1 ";
$sql.=")a order by DealerName";
}
else
{
 ...
 foreach ($query->result() as $row)
	...
	public function getOrderCategoryDropdownListNew()//need to be changed
	{
		 ...
		 if (isset($postdata))
		  ...
		  $query=$this->db->query("SELECT PK_CategoryID,CategoryName FROM `tbl_master_productcategory` where IsActive=1 and IsDeleted=0");
