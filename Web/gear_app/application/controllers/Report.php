 ...
 { 
  ...
  redirect('', 'refresh');
	...
	public function visitreport()
	{
        ...
        $this->load->view('datePickerJsReport');
...
if(empty($rsarray))
                {
                    ...
                    $sql="SELECT c.Company_name,d.cwExecutiveName,d.DealerName FROM `tbl_master_company` c,`tbl_master_dealerinformation` d where c.PK_CompanyID=d.FK_CompanyID  and d.cwExecutiveName not like 'Not Applicable' ";
              
              if($companyID>0)
                 $sql.=" and d.FK_CompanyID=".$companyID;

               $sql.="  group by c.Company_name,d.cwExecutiveName,d.DealerName order by Company_name,cwExecutiveName,DealerName";
