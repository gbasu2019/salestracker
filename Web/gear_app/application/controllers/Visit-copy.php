...
class Visit extends CI_Controller {
	...
	public function saveventry($userid,$dealerid,$latitude,$longitude,$output_file)
	{
		$this->load->model('JbCommon');
		 $address=$this->Get_Address_From_Google_Maps($latitude, $longitude);	

if (empty($address)	) $this->saveventry($userid,$dealerid,$latitude,$longitude,$output_file);
		  ...
		  if (isset($postdata))
			{
				 ...
				 $output_file="";
				 if(isset($request->filename_base64))
				 {
					
					 $filename="c.jpg";
					 $image_base_64_string=$request->filename_base64;
					 
					 //$visitid=$this->db->select_max('PK_VisitID')->get('tbl_trans_visitentry')->row();
					 //$coll=$visitid->PK_VisitID + 1;	
								 
					$sql1="SELECT `DealerName` as Name FROM `tbl_master_dealerinformation` WHERE PK_DealerID=".$dealerid;	
					$query4 = $this->db->query($sql1);
					$row1=$query4->row();
					$Name=$row1->Name;		
					$col1=date('YmdHis').''.$Name;		
				...
				$this->saveventry($userid,$dealerid,$latitude,$longitude,$output_file);
	...
	{
       //$key="AIzaSyD8Y5p1k5dso5WhMoG7jenHzHi72g9sndI";
       $url = "http://maps.googleapis.com/maps/api/geocode/json?latlng=".$lat.",".$lon."&sensor=false&key==AIzaSyD8Y5p1k5dso5WhMoG7jenHzHi72g9sndI";
